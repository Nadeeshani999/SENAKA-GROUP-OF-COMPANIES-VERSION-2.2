...
const Charts = {
  ...
  bar(id, labels, datasets, opts={}) {
    ...
    if (opts.log) {
      const logAxis=opts.horizontal?'x':'y';
      cfg.scales[logAxis].type='logarithmic';
      cfg.scales[logAxis].min=1;
    }
...
async function initBankPage() {
  ...
  function renderView(rows) {
    ...
    const sorted=[...rows].sort((a,b)=>Math.abs(b['Amount']||0)-Math.abs(a['Amount']||0));
    const bLabels=sorted.map(r=>abbrevBank(r['Bank Name']||''));
    const bData=sorted.map(r=>Math.max(1,Math.abs(r['Amount']||0)));
    Charts.bar('bank-bar',bLabels,[{label:'Balance (LKR — log scale)',data:bData,colors:sorted.map(r=>(r['Amount']||0)>=0?C.primary+'cc':C.red+'cc')}],{horizontal:true,currency:true,log:true});
...
async function initSleepersPage() {
  ...
  function renderView(rows) {
    ...
    // Chart 1 — Combined Production Progress (stacked horizontal bar: Casted vs Balance per tender)
    const selStatus=statusEl?.value;
    ...
    // Chart 3 — For single ongoing: key qty metrics bar; otherwise stacked by site
    if (ogRows.length===1&&selStatus!=='Completed') {
      const t=ogRows[0];
